...
void SpecificWorker::compute()
{
    ...
    {
        target = data.value();
        propagar(std::get<0>(target),std::get<2>(target));
...
void SpecificWorker::propagar(int x, int z){
    grid.reset_cell_distances();
    int dist = 0;
    auto listA = grid.vecinos(grid.get_value(x,z),dist);
    std::vector<MyGrid::Value> listB;
    bool end = false;

    while(not end){
        for(auto &current_cell: listA){
            auto selected = grid.vecinos(current_cell,dist);
            listB.insert(listB.end(),selected.begin(),selected.end());
        }
        dist++;
        end = listB.empty();
        listA.swap(listB);
...
void SpecificWorker::dynamicWindowApproach(RoboCompGenericBase::TBaseState bState, RoboCompLaser::TLaserData &ldata) {
    ...
    if (dist < 300)
    {
        ...
        tupla Front = ordenar(vectorSInObs, tr.x(), tr.y());

        if(vectorSInObs.size() > 0) {
            auto[x, y, v, w, alpha] = Front;
            ...
            draw_things(bState, ldata, vectorSInObs, Front);
        }else{
            std::cout << "Vector vacio" << std::endl;
            if(dist > 750){//Si el robot se mete en una esquina, al quitar todos los puntos que le llevan a obstaculos en el dynamicWindowsAproach,
                // se queda vacio el vector, y no calcula nada, por lo que hacemos un giro y de esta forma recuperamos puntos del laser sobre el mapa.
 ...
 */
    std::vector <SpecificWorker::tupla> SpecificWorker::calcularPuntos(float vOrigen, float wOrigen) {
        ...
        for (float dt = 0.2; dt < 1.5; dt += 0.1) { //velocidad robot
            for (float v = -100; v <= 700; v += 80) //advance
            {
 ...
 */
    ...
    std::vector <tuple<tupla, float>> Resultados;
    SpecificWorker::tupla Front;
    float euclideo = 0, peso;
    for (auto &t : vector){
        auto x2 = (std::get<0>(t));
        auto z2 = (std::get<1>(t));

        auto [k, l] = grid.transformar(x2, z2); //Comprobamos si clickamos dentro del mapa
        if (grid.is_in_limits(k, l) ){
            euclideo = std::sqrt((x2 - x) * (x2 - x) + (z2 - z) * (z2 - z));
            auto v = this->grid.get_value(x2, z2);
            peso = v.dist * 1 + euclideo * 0.05;
            Resultados.insert(Resultados.end(), std::make_tuple(t, peso));
        }//if is in limits
    }//for
    std::sort(Resultados.begin(), Resultados.end(), [x, z](const auto &a, const auto &b){
        return std::get<1>(a) < std::get<1>(b);
    } );

    if( not Resultados.empty() ) {
        Front = (std::get<0>(Resultados.front()));
        /*std::cout<< "\n\n"<< "Euclideo --> "<< std::sqrt((std::get<0>(Front) - x)*(std::get<0>(Front) - x) + (std::get<1>(Front) - z)*(std::get<1>(Front) - z)) <<std::endl;
        std::cout << "Dist -->" << grid.get_value(std::get<0>(Front), std::get<1>(Front)).dist << std::endl;
        std::cout << "peso -->" << std::get<1>(Resultados.front()) << "\n" <<std::endl;
         */
    }
